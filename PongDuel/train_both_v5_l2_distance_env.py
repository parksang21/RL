...
class DDQNAgent:  # Double Deep Q-Network
    def __init__(self, model, target_model, env, buffer_size=1000, learning_rate=.0015, epsilon=0.5, epsilon_dacay=0.999,
                 min_epsilon=.01, gamma=.9, batch_size=64, target_update_iter=10, train_nums=5000, start_learning=5,):

        self.model_p = model
        self.target_model_p = target_model

        self.model_q = Model(3)
        self.target_model_q = Model(3)
    ...
    def train(self):
        ...
        for t in range(1, self.train_nums):
            ...
            if t % 100 == 0:
                self.model_p.save_weights('./saved_weights5/{}.ckpt'.format(t))
    ...
    def train_step(self):
        ...
        losses_q = self.model_p.train_on_batch(s_batch, target_f_q)
