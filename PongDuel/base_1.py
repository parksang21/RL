...
class Model(tf.keras.Model):
    def __init__(self):
        ...
        self.fc1 = kl.Dense(32, activation='relu', kernel_initializer='he_uniform', input_dim=(2,10))
...
class DDQNAgent:  # Double Deep Q-Network
    def __init__(self, model, target_model, env, buffer_size=200, learning_rate=.0015, epsilon=.1, epsilon_dacay=0.995,
        ...
        self.obs = np.empty((self.buffer_size,) + (2, 10))
        ...
        self.next_states = np.empty((self.buffer_size,) + (2, 10))
        self.next_idx = 0

        self.player = 0 if player == "Left" else 1

    def train(self):
        # initialize the initial observation of the agent
        obs = np.asarray(self.env.reset())
        for t in range(1, self.train_nums):
            best_action, q_values = self.model.action_value(obs)  # input the obs to the network model
    ...
    def train_step(self):
        ...
        print(np.arange(target_q.shape[0]))

        target_q = r_batch + self.gamma * target_q[np.arange(target_q.shape[0]),:,best_action_idxes.T] * (1 - done_batch)

        target_f = self.model.predict(s_batch)
        for i, val in enumerate(a_batch):
            print("tf:",target_f[i][val])
            print("val", val)
            print("tq", target_q)
    ...
    def evalation(self, env, render=True):
        obs, done, ep_reward = env.reset(), False, [0, 0]
        # one episode until done
        while not done:
            action, q_values = self.model.action_value(np.asarray(obs))  # Using [None] to extend its dimension (4,) -> (1, 4)
...
if __name__ == '__main__':
    ...
    print("After Training: {} out of 200".format(rewards_sum)) # 9 out of 200

    agent.train()
    # test after
    # env = gym.wrappers.Monitor(env, './recording', force=True)  # to record the process
    rewards_sum = agent.evalation(env)
    print("After Training: {} out of 200".format(rewards_sum)) # 200 out of 200