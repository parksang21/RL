...
class DDQNAgent:  # Double Deep Q-Network
    def __init__(self, model, target_model, env, buffer_size=200, learning_rate=.0015, epsilon=.1, epsilon_dacay=0.995,
        ...
        self.next_idx = 0
        self.player, self.opponent = [0, 1] if player == 'Left' else [1, 0]
    ...
    def train(self):
        ...
        for t in range(1, self.train_nums):
            ...
            next_obs, reward, done, info = self.step(action)    # take the action in the env to return s', r, done
    ...
    def evalation(self, env, render=True):
        ...
        while not done:
            action, q_values = self.model.action_value(obs[self.player][None])  # Using [None] to extend its dimension (4,) -> (1, 4)
...
if __name__ == "__main__":
    ...
    agent = DDQNAgent(model, target_model, env, player='Left')
    # # test before
