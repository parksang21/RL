...
class DDQNAgent:  # Double Deep Q-Network
    def __init__(self, model, target_model, env, buffer_size=1000, learning_rate=.0015, epsilon=0.9, epsilon_dacay=0.999,
        ...
        for t in range(1, self.train_nums):
            ...
            while not done[0] and step_count < 100000:
                ...
                if np.sum(ball_d[:3]) == 1:
                    reward[0] = -10 * (abs(obs[0] - ball_position[0]) + abs(obs[1] - ball_position[1]))

                # right player에게 향하고 있다.
                elif np.sum(ball_d[-3:]) == 1:
                    reward[1] = -10 * (abs(obs[10] - ball_position[0]) + abs(obs[11] - ball_position[1]))
    ...
    def train_step(self):
        ...
        best_action_idxes, _ = self.model_p.action_value(ns_batch)
