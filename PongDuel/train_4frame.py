...
class DDQNAgent:  # Double Deep Q-Network
    def __init__(self, modelp, target_modelp, modelq, target_modelq, env, buffer_size=200, learning_rate=.0015, epsilon=.1, epsilon_dacay=0.995,
                 min_epsilon=.01, gamma=.9, batch_size=64, target_update_iter=100, train_nums=10000, start_learning=100,):
    ...
    def train(self):
        ...
        for t in range(1, self.train_nums):
            ...
            while not done[0] and step_count < 100000:
                ...
                self.store_transition(obs_window, action, reward, n_obs_window, done)  # store that transition into replay butter
                ...
                # print('EPISODE {:>5}-step{} Epsilon {:>1.10} losses: {}, rewards: {}'.format(t, step_count, self.epsilon, losses, rewards))
            ...
            if t % 100 == 0:
                self.model_p.save_weights('./4frame3/model_p{}.ckpt'.format(t))
                self.model_q.save_weights('./4frame3/model_q{}.ckpt'.format(t))
                self.target_model_p.save_weights('./4frame3/target_model_p{}.ckpt'.format(t))
                self.target_model_q.save_weights('./4frame3/target_model_q{}.ckpt'.format(t))
