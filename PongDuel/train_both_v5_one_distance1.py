...
class DDQNAgent:  # Double Deep Q-Network
    def __init__(self, model, target_model, env, buffer_size=1000, learning_rate=.0015, epsilon=0.9, epsilon_dacay=0.998,
        ...
        for t in range(1, self.train_nums):
            ...
            while not done[0] and step_count < 100000:
                ...
                if reward[0] == 1:
                    ...
                    reward[0] = -10 * np.sqrt((obs[0] - ball_position[0])*(obs[0] - ball_position[0]))
                    reward[1] = -10 * np.sqrt((obs[10] - ball_position[0])*(obs[10] - ball_position[0]))
            ...
            if t % 100 == 0:
                self.model_p.save_weights('./saved_weights/only_distance{}.ckpt'.format(t))
